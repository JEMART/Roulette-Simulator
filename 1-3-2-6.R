...
#the 1-3-2-6 system while tracking the money meanwhile
...
F1326<-function(b,c,x){  #b= amount of min money bet (unit)   c=colour bet on in the first spin, x=total number of spins
      ...
      for (i in 2:x){   #create money bet in the ongoing cycle vector for all rows
          z[i,5]<-ifelse(z[i-1,4]==1,ifelse(z[i-1,5]==b,b*3,ifelse(z[i-1,5]==b*3,b*2,
                  ifelse(z[i-1,5]==b*2,b*6,b))),b)
      }  
